...
def main(args):   
    ...
    for i in range(Max_Ite):
        ...
        if levenshteinDistance(args.ttext, transcription[0])==0:
            ...
            sf.write('{}/best_at_{}.wav'.format(audio_path, int(i)), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV') 
            print('The {} attack is successful at iteration {}'.format(args.index, i))
            break
        if i%200==0:
            sf.write('{}/audio_{}.wav'.format(audio_path, int(i)), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV') 
            print('ite {}:  {} {} {}'.format(i, loss.cpu().data.numpy(), transcription[0], levenshteinDistance(args.ttext, transcription[0])))
    plt.style.use('bmh')
    plt.figure()
