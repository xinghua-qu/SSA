...
def adam_lr(args, step):
    b1 = 0.9
    b2 = 0.999
    bc1 = 1/(1-b1**step)
    bc2 = 1/(1-b2**step)   
    alr = args.init*bc1/np.sqrt(bc2)
    return alr
...
#     optimizer = optim.Adam(my_model.n2, lr=args.init,
...
#     scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200)
    ...
    VAR_n2  = []
    noise_2 = n_2
    figure_path = 'results/{}/figures/{}/target_{}'.format(args.data_set, args.ctext, args.ttext)
    if not os.path.exists(figure_path):
        os.makedirs(figure_path)
    audio_path = 'results/{}/audio/{}/target_{}'.format(args.data_set, args.ctext, args.ttext)
    if not os.path.exists(audio_path):
        os.makedirs(audio_path)
    data_path = 'results/{}/saved_data/{}/target_{}'.format(args.data_set, args.ctext, args.ttext)
    if not os.path.exists(data_path):
        os.makedirs(data_path)
    
    if os.path.exists('{}/data_{}.csv'.format(data_path, args.index)):
          os.remove('{}/data_{}.csv'.format(data_path, args.index))
    head = ['CTC loss', 'Conditional Text', 'Transcription ({})'.format(args.ttext), 'Learning rate', 'Mean of noise', 'Variance of noise', 'noise', 'audio']
