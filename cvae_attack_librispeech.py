...
def myargs():
    ...
    parser.add_argument('--save_name', type=str, default = 'common_voice', help='name for saving results')
    ...
    parser.add_argument('--sid', type=float, default = 18, help='the speaker id for multi-speaker TTS model (In Single speaker version, sid=None)')
    ...
    parser.add_argument('--ls', type=float, default = 1.35, help='length scale for duration predictor')
    ...
    parser.add_argument('--runid', type=int, default = 0, help='the index for run id')
...
def read_csv(dataset):
    if dataset=='common_voice':
        name = 'data_sets/common_voice/common_voice_cvae_attack_data_MOS.csv'
    else:
    ...
    with open(name) as tsvfile:
        ...
        for line in tsvreader:
            ...
            if ind>=200: # 100 is the length of the test dataset
                break
            ind += 1
...
class CVAE_Attack_Net(nn.Module):
    ...
    def forward(self, n2):
        ...
        reg = 100*(torch.abs(torch.abs(mean_n2)-0.0001)+torch.abs(var_n2-1))
...
def main(args):   
    ...
    VAR_n2  = []
    noise_2 = n_2
    print('case {}: {} '.format(args.index, n_2.size()))
    figure_path = 'results/{}/figures/case_{}'.format(args.save_name, args.index)
    if not os.path.exists(figure_path):
        os.makedirs(figure_path)
    audio_path = 'results/{}/audio/case_{}'.format(args.save_name, args.index)
    if not os.path.exists(audio_path):
        os.makedirs(audio_path)
    data_path = 'results/{}/saved_data/case_{}'.format(args.save_name, args.index)
    if not os.path.exists(data_path):
        os.makedirs(data_path)
    
    if os.path.exists('{}/data_{}.csv'.format(data_path, args.index)):
          os.remove('{}/data_{}.csv'.format(data_path, args.index))
    head = ['CTC loss', 'Conditional Text', 'Transcription ({})'.format(args.ttext), 'Learning rate', 'Mean of noise', 'Variance of noise', 'noise']
    with open('{}/data_{}.csv'.format(data_path, args.index), 'a+') as f:
        writer = csv.writer(f)
    ...
    for i in range(Max_Ite):
        loss, transcription = my_model(noise_2)
        loss.backward()
        if i==0:
            history_best = loss
        if loss<history_best:
            history_best = loss
            patience = 0
        else:
            patience += 1
            patience, alpha = plateau_decay(patience, alpha, args)
        Convergence.append(loss.cpu().data.numpy())
        with torch.no_grad():
            if torch.isnan(my_model.n2.grad).any():
                my_model.n2.grad = torch.where(torch.isnan(my_model.n2.grad), torch.randn(m_p.size(), dtype=m_p.dtype, layout=m_p.layout, device=m_p.device, requires_grad=True), my_model.n2.grad)
            updata_grads = torch.sign(my_model.n2.grad)
            noise_2_update = noise_2 - alpha*updata_grads
        noise_2 = torch.clamp(noise_2_update, -args.scale_bound, args.scale_bound)
        row = [loss.cpu().data.numpy(), args.ctext, transcription[0], alpha, noise_2.mean().cpu().data.numpy(), noise_2.var().cpu().data.numpy(), noise_2.cpu().data.numpy(), my_model.audio.cpu().data.float().numpy()]
        with open('{}/data_{}.csv'.format(data_path, args.index), 'a+') as f:
            writer = csv.writer(f)
            writer.writerow(row) 
        if levenshteinDistance(args.ttext, transcription[0])==0:
            print('ite {}:  {} {}'.format(i, loss.cpu().data.numpy(), transcription[0]))
            sf.write('{}/adversarial.wav'.format(audio_path), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV') 
            print('The {} attack is successful at iteration {}'.format(args.index, i))
            break
        if i==0:
            sf.write('{}/original.wav'.format(audio_path), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV')
        if i%200==0:
            sf.write('{}/in_the_middle.wav'.format(audio_path), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV')
        if i==Max_Ite-1:
            sf.write('{}/adversarial.wav'.format(audio_path), my_model.audio.cpu().data.float().numpy()[0], hps.data.sampling_rate, format='WAV') 
            print('final ite {}:  {} {} {}'.format(i, loss.cpu().data.numpy(), transcription[0], levenshteinDistance(args.ttext, transcription[0])))
    plt.style.use('bmh')
    plt.figure()
    plt.plot(Convergence)
    plt.xlabel('iteration')
    plt.ylabel('CTC Loss')
    plt.title("The convergence of CTC loss for CVAE attack")
    plt.savefig('{}/Loss_convergence_case_{}.pdf'.format(figure_path, args.index))
    
    plt.style.use('bmh')
    plt.figure()
    plt.plot(np.reshape(n_2.cpu().data.float().numpy()[0,0:2,:], [-1]), label='Original')
    plt.plot(np.reshape(noise_2.cpu().data.float().numpy()[0,0:2,:], [-1]), label='Adversarial')
    plt.xlabel('Dimension of audio style vector '+r'$z$')
    plt.ylabel('Value of '+r'$z$')
    plt.legend()
    plt.savefig('{}/z_vector_{}.pdf'.format(figure_path, args.index))

def FUN(i):
    torch.set_default_tensor_type(torch.DoubleTensor)
    args       = myargs()
    os.environ["CUDA_VISIBLE_DEVICES"]="{}".format(args.gpu)
    args.data_set = 'common_voice'
    args.save_name = 'ICLR_Rebuttal_{}_init_{}_patience_{}'.format(args.data_set, args.init, args.patience)
    CText, TText = read_csv(args.data_set)
    if len(CText)!=len(TText):
        raise ValueError('The dataset is not loaded correctly')
    args.index = i+5*args.runid
    args.ctext = CText[args.index]
    args.ttext = TText[args.index]
    main(args)
